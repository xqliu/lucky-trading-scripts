...
## 检查清单（PR/改动前）— 递归执行直到 0 问题

**此清单必须递归执行：发现任何问题 → 修复 → 从头重新走一遍。只有全部 ✅ 才能 commit/报完成。**
...
- [ ] 价格是否 round？
- [ ] 类似函数是否同步检查了？
- [ ] `pytest tests/` **全量 0 failed**？（不是"N passed"，是"0 failed"。有 fail 必须查明原因，不能说"历史遗留"跳过）
- [ ] 改了 config/环境变量/路径？→ 检查所有从同一源解析的路径是否仍然安全（特别是 state 文件写入位置）
